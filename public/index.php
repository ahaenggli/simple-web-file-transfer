...
function uploadlinks($key=null){
  ...
  else $ret = $db->query('select * from links where type="u" and key="'.$key.'";');
...
function add_uploadlink($key, $value){
  global $db;
  $sql ='INSERT INTO links (type,key,value)  VALUES ("u","'.$key.'","'.$value.'"); ';
  $ret = $db->exec($sql);
  if(!$ret) {
...
function remove_uploadlink($key){
  global $db;
  $sql ='delete from links where type="u" and key = "'.$key.'";';
  $ret = $db->exec($sql);
  if(!$ret) {
      echo $db->lastErrorMsg();
  } 
...
function downloadlinks($key=null){
  ...
  else $ret = $db->query('select * from links where type="d" and key="'.$key.'";');
...
function add_downloadlink($key, $value){
  global $db;
  $sql ='INSERT INTO links (type,key,value)  VALUES ("d","'.$key.'","'.$value.'"); ';
  $ret = $db->exec($sql);
  if(!$ret) {
      echo $db->lastErrorMsg();
  } 
...
function remove_downloadlink($key){
  global $db;
  $sql ='delete from links where type="d" and key = "'.$key.'";';
  $ret = $db->exec($sql);
  if(!$ret) {
      echo $db->lastErrorMsg();
  } 
...
<script type="text/javascript" src="<?php echo $sso_str;?>/webman/sso/synoSSO-1.0.0.js" async></script>
<script>
